...
from trader.models.currency_strategy import CurrencyStrategyImplementationType
...
@dataclass
class CurrencyStrategyImplementationTypeData:
    cache_key: str
    description: str


CURRENCY_OHLCV = CurrencyStrategyImplementationTypeData(
    "currency_strategy_implementation_type_currency_ohlcv_id", "Currency OHLCV"
)
CURRENCY_STRATEGY_IMPLEMENTATION_TYPES = (CURRENCY_OHLCV,)


def initialize_currency_strategy_implementation_types(session: Session) -> None:
    for currency_strategy_implementation_type in CurrencyStrategyImplementationTypeData:
        instance = (
            session.query(CurrencyStrategyImplementationType)
            .filter_by(description=currency_strategy_implementation_type.description)
            .one_or_none()
        )
        if not instance:
            instance = CurrencyStrategyImplementationType(description=currency_strategy_implementation_type.description)
            session.add(instance)
            session.flush()
        cache.set(currency_strategy_implementation_type.cache_key, instance.id)
...
def initialize_base_data() -> None:
    with DBSession() as session:
        ...
        initialize_currency_strategy_implementation_types(session)
        session.commit()
