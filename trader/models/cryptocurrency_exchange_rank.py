from sqlalchemy import Column, DateTime, ForeignKey, Integer, Numeric, SmallInteger
...
class CryptocurrencyExchangeRank(Base):
    ...
    spot_vol_24h = Column(Numeric, nullable=True)
    derivatives_vol_24h = Column(Numeric, nullable=True)
    derivatives_open_interests = Column(Numeric, nullable=True)
    weekly_web_visits = Column(Integer, nullable=True)
    market_share_percentage = Column(Numeric, nullable=True)
    maker_fee = Column(Numeric, nullable=False)
    taker_fee = Column(Numeric, nullable=False)
