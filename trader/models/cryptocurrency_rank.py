from sqlalchemy import Boolean, Column, DateTime, ForeignKey, Integer, Numeric, SmallInteger
...
class CryptocurrencyRank(Base):
    ...
    usd_market_cap = Column(Numeric, nullable=True)
    usd_price = Column(Numeric, nullable=False)
    usd_volume_24h = Column(Numeric, nullable=False)
    circulating_supply = Column(Numeric, nullable=True)
    total_supply = Column(Numeric, nullable=True)
