...
from trader.utilities.constants import EnabledQuoteAsset, INITIAL_ENABLED_QUOTE_CRYPTOCURRENCIES, INITIAL_ENABLED_QUOTE_STANDARD_CURRENCIES
...
def set_enabled_quote_asset(session: Session, asset_type_id: int, enabled_quote_asset: EnabledQuoteAsset) -> None:
    asset = (
        session.query(Asset)
        .filter_by(asset_type_id=asset_type_id, symbol=enabled_quote_asset.symbol)
        .one_or_none()
    )
    if asset:
